...
{ TJsonNode is the class used to parse, build, and navigate a json document.
  ...
  public
    ...
    function items(Index: Integer): TJsonNode;
    ...
    { Convert the node to a float }
    property AsFloat: Double read GetAsFloat write SetAsFloat;
    { Convert the node to a integer }
...
function TJsonNode.GetAsFloat: Double;
begin
  if FParent = nil then
    Error(SRootNodeKind);
  if FKind <> nkNumber then
...
procedure TJsonNode.SetAsFloat(Value: Double);
begin
  if FParent = nil then
    Error(SRootNodeKind);
  if FKind <> nkNumber then
...
begin
  ...
  Result := trunc(StrToFloatDef(FValue, 0));
end;

procedure TJsonNode.SetAsInteger(Value: integer);
begin
  if FParent = nil then
    Error(SRootNodeKind);
  if FKind <> nkNumber then
...
function TJsonNode.items(Index: Integer): TJsonNode;
begin
  if FKind in [nkArray, nkObject] then
  begin
    if FList = nil then
